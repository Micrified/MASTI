            ...
            for line in file:
                if line.startswith("Resolver:"):
                    if no != 0:
                        mean = mean/no
                        writer.writerow(
                            [resolver, round(minimum, 2), round(mean, 2), round(maximum, 2)])
                        minimum = None
                        maximum = None
                        mean = 0
                        no = 0
...
if __name__ == "__main__":
    ...
    # # Open a file
    # TLDS = open("../tld.txt", "rw+")

    # line = TLDS.readline()
    # print "Read Line: %s" % (line)
    TLDS=["com", "net", "br", "nl", "ca"]
    for tld in TLDS:
        print (DNSSec_path+"Results/resultdot"+tld+".txt", "DSec"+tld)
        process_txt(DNSSec_path+"Results/resultdot"+tld+".txt", "DSec"+tld)
            

    #process_txt(DoH_path+"Results/resultdotcom.txt", "DoH")
    #process_txt(DNSSec_path+"Results/resultdotcom.txt", "DSeC")

    r, minlist, avglist, maxlist = process_csv("resultsDSeCcom.csv")
    rT = tuple(r)
    minT = tuple(minlist)
    avgT = tuple(avglist)
    maxT = tuple(maxlist)
