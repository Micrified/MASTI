...
{
  fprintf(stderr,
          "Usage: doh [options] <host> [no of resolver]\n"
          "  -h  this help\n"
          "  -v  verbose mode\n");
  exit(0);
}
...
int main(int argc, char **argv)
{
  ...
  int trace_enabled = 0;
  int rc;
  const char *resolvers[16];

  resolvers[0]="https://dns.google.com/experimental";
  ...
  resolvers[10]="https://commons.host";
  resolvers[11]="https://doh.dnswarden.com/adblock";
  resolvers[12]="https://dns-gcp.aaflalo.me/dns-query";
  resolvers[13]="https://dns.aaflalo.me/dns-query";
  resolvers[14]="https://doh.appliedprivacy.net/query";
  resolvers[15]="https://doh.captnemo.in/dns-query";
  ...
  if(argc > 1) {
    ...
    help();
     host = argv[url_argc];
  if(argc > 1 + url_argc)
     resolver_no = atoi(argv[url_argc + 1]) - 1;
  ...
  doh_init(&d);
  initprobe(&probe[0], DNS_TYPE_A, host, resolvers[resolver_no], multi, trace_enabled, headers);
  initprobe(&probe[1], DNS_TYPE_AAAA, host, resolvers[resolver_no], multi, trace_enabled, headers);
